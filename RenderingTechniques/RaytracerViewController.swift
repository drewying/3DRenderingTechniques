...
class RaytracerViewController: UIViewController {
    ...
    var sceneObjects: [Sphere] = [Sphere]()
    var colorBuffer: [Color] = [Color]()
    ...
    func drawScreen() {
        for col in 0..<renderView.width {
            for row in 0..<renderView.height {
                //Generate a ray that passes through the pixel at (x, y)
                let ray: Ray = makeRayThatIntersectsPixel(col: col, row: row)
                ...
                let currentColor = colorBuffer[row * renderView.width + col]
                let mixedColor = ((currentColor * Float(samplenumber)) + newColor)  *  (1.0/Float(samplenumber + 1))
                colorBuffer[row * renderView.width + col] = mixedColor
                renderView.plot(x: col, y: row, color: mixedColor)
            }
        }
    }
    ...
    func makeRayThatIntersectsPixel(col: Int, row: Int) -> Ray {
        ...
        let dcol = 1.0 / Float(renderView.width)
        let drow = 1.0 / Float(renderView.height)

        var cameraX = (2 * (Float(col) + 0.5) * dcol - 1) * aspectRatio * scale
        var cameraY = (1 - 2 * (Float(row) + 0.5) * drow) * scale * -1

        //Randomly move the ray slightly up or down to create anti-aliasing
        let randomX = Float(arc4random()) / Float(UINT32_MAX)
        let randomY = Float(arc4random()) / Float(UINT32_MAX)
        cameraX += (randomX - 0.5)/Float(renderView.width)
        cameraY += (randomY - 0.5)/Float(renderView.height)
